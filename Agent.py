...
def call_llm(messages: list[dict]) -> str:
    """
    Call the OpenAI API with a history of messages and return the assistant's reply.
    Implements retry logic to handle rate limit errors.
    """
    while True:
        try:
            response = openai.ChatCompletion.create(
                model="gpt-4o",  # Use a model you have access to
                messages=messages,
                temperature=0.2,
                max_tokens=500,
            )
